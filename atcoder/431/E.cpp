...
void solve() {
	...
	Stree just;
	just.init(500001, cnt);
	just.build(0, 500000, 1);
	// // cout<<just.segtree[1].cnt<<endl;
	while (q--) {
		ll type; cin >> type;
		ll l, r; cin >> l >> r;
		if (type == 1) {
			l--;
			cnt[a[l]] --;
			just.update(a[l], cnt[a[l]], 0, 500000, 1);
			a[l] = r;
			cnt[a[l]]++;
			just.update(a[l], cnt[a[l]], 0, 500000, 1);
		} else {
			...
			if (r >= l) {
				auto node = just.query(l, r, 0, 500000, 1);

				ll ans  = node.sum + just.query(0, l - 1, 0, 500000, 1).cnt * l + just.query(r + 1, 500000, 0, 500000, 1).cnt * r;
				cout << ans << '\n';
				// ll fuck = 0;
				// for (ll i = 0; i < n; i++) {
				// 	// fuck += max(l, min(a[i], r));
				...
				// }
				// cout << fuck << ' ' << just.query(0, l - 1, 1, 500000, 1).cnt * l  << '\n';
			}
			else {
				cout << l * n << '\n';
			}
