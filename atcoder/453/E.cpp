...
void solve() {
	...
	for (ll i = 0; i < n; i++) {
		mp[jus[i].first].push_back(jus[i].second);
	}
	...
	Mint ans = 0;
	for (ll i = 1; i < n; i++) {
		for (auto pos : mp[i])temp.insert(pos);

		ll oth = n - i;
		ll com ;
		if (oth >= i) {
			com = temp.size()  - temp.order_of_key(oth);
			done[ {i, oth}] = com;
		} else {
			com  = done[ {oth, i}];
		}
