...
struct Stree
{
    ...
    void propagate(ll tl,ll tr,ll node){
        ...
        if(op==0){
            ...
            segtree[2*node].B=segtree[2*node].L=segtree[2*node].Bprefix=segtree[2*node].Bsuffix=0;
            segtree[2*node].L1=left;
            
            
            segtree[2*node+1].B=segtree[2*node+1].L=segtree[2*node+1].Bprefix=segtree[2*node+1].Bsuffix=0;
            ...
            segtree[2*node].B=segtree[2*node].Bprefix=segtree[2*node].Bsuffix=1;
            segtree[2*node].L1=0;
            segtree[2*node].L=left;
            
            
            segtree[2*node+1].B=segtree[2*node+1].Bprefix=segtree[2*node+1].Bsuffix=1;
    ...
    void update(ll l, ll r,ll type,ll tl, ll tr, ll node)
    {
        ...
        if (tl >= l && tr <= r)
        {
            if(type==0){
                ...
                segtree[node].B=segtree[node].L=segtree[node].Bprefix=segtree[node].Bsuffix=0;
